...
def _run(steps, dispatch, **kwargs):
    kwargs.setdefault("persist_log", False)
    return asyncio.run(run_steps(steps, dispatch, **kwargs))
...
def test_run_steps_validates_limits_and_output_mode():
    ...
    with pytest.raises(StepRunnerError):
        _run([], dispatch)
    with pytest.raises(StepRunnerError):
        _run([{"tool": "x"}] * (MAX_STEPS + 1), dispatch)
    with pytest.raises(StepRunnerError):
        _run([{"tool": "x"}], dispatch, output_mode="giant")
...
def test_compact_output_trims_large_intermediate_text():
    ...
    result = _run([{"tool": "shell"}], dispatch)
...
def test_full_output_preserves_large_intermediate_text():
    ...
    result = _run([{"tool": "shell"}], dispatch, output_mode="full")
